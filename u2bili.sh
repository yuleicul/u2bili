...
yt-dlp "$yturl" --quiet --write-thumbnail --write-auto-subs --sub-langs en,zh-Hans --sub-format srt  -o "${downloadPath}%(id)s.%(ext)s" --exec "node upload.js ${downloadPath}$vid.json"